...
const GREENHOUSE_MAX = 0.55; // strongest greenhouse multiplier contribution.
...
function orbitalRadiusFor(star: StarProfile, index: number): number {
  let a = 0.25 * Math.sqrt(star.luminosity) + 0.15; // index 0 sits at the inner edge
  for (let k = 1; k <= index; k++) {
    const slotRng = makeRNG(deriveSeed(star.seed, 0x0a17 + k));
    a *= 1.3 + slotRng() * 0.7; // spacing in [1.3, 2.0)
  }
  return a;
}
...
export function derivePlanet(star: StarProfile, index: number): PlanetProfile {
  ...
  const waterFraction = clamp01(rangeFloat(rng, 0, 1) * (isGiant ? 0.2 : 1));
